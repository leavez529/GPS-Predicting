...
def calError(pred,true):
    ...
    for i in range(len(pred)):
        ...
        for j in range(len(pred_track)):
            p_id = pred_track[j]
            t_point = true_track[j]
            p_center = calCenter(p_id)
            p_u = utm.to_latlon(p_center[0],p_center[1],51,'R')
            e = calMeters([p_u[1],p_u[0]],t_point)
            print(e)
            error.append(e)
    error = np.sort(error)
    print(error)
...
def trainModel(ismi_set):
    ...
    model = keras.models.load_model("./f88.LSTM")

    sequence_autoencoder = keras.models.load_model('./autoencoder88')
    seq_test = sequence_autoencoder.predict(x_test)
    print(seq_test)
    y_pred = model.predict_classes(seq_test)
